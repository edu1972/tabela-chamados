...
class AplicacaoChamados:
    def __init__(self, root):
        ...
        self.label_numero.place(x=10,y=10) #pack(side='top') troca do pack pelo place pata posicionar melhor os botões
        self.entry_numero = ttk.Entry(self.root)
        self.entry_numero.place(x=150,y=10) #pack(side='top')

        self.label_tipo = ttk.Label(self.root, text="Tipo do Chamado:")
        self.label_tipo.place(x=10,y=40)
        self.combo_tipo = ttk.Combobox(self.root, values=[ "Normal","Urgente"])
        self.combo_tipo.place(x=150,y=40)
                
        self.label_tecnico = ttk.Label(self.root, text="Técnico Designado:")
        self.label_tecnico.place(x=10,y=70)
        self.combo_tecnico = ttk.Combobox(self.root, values=["Anderson", "Daniel", "Douglas", "Fábio", "Giovanni", "Jean", "Jonathan", "Leonardo", "Eduardo", "Marcelo", "Marcos", "Vinicius"])
        self.combo_tecnico.place(x=150,y=70)

        # Botoes ajustados um a um com a tag place(x=>coluna, y=> linha)
        # Adicionado label do desenvolvedor, gostei desta aplicação.
        self.label_dev = ttk.Label(self.root, text="Desenvolvido por Eduardo Machado")
        self.label_dev.place(x=1200,y=380) 
        ...
        self.botao_adicionar.place(x=180,y=100)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.geometry('1400x400') #definir largura da aplicaçao, provisório.
    aplicacao = AplicacaoChamados(root)
    root.mainloop()

## Anotações de melhoria:
    
    # Botão remover técnico,criar aplicação para adicionar outro técnico;
    # Escolher técnico por ordem da tabela, para as duas situações de chamados, Normal e Urgente;
    # Centralizar as informações adicionadas no 'TreeView', procurar um 'center text' se tiver ou equivalente;
    # Fazer uma aplcação responsiva;
    # Pesquisar sobre empacotamento para instalar a aplicação em outros PC's sem necessariamente ter o "Python instalado";
    # Pesquisar sobre background cores e imagens e botões;
